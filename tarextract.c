...
#define DIRECTORY '5'
#define OCT 8
/*a function that sets the times of the file*/
...
/*a function that extracts the files from a tar archive*/
int extract(char *archivename, char **targets, int numtargets) {
    ...
    while(read(fdTar, &fileheader, 512) == 512) {
        ...
        if(isheadernull(&fileheader)) {
            ...
            /*checking if its a valid tar file*/
        ...
        /*this check block checks for targets 
        and makes sure the proper directories are created*/
